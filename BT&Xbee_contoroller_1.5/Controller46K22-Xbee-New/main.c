...
#if 1
#define Xbee_spb (1.0/57600.0);
#define CLK_IN_TMR3 (64000000.0/4.0)
#define CNT_per_ONE_BIT (Xbee_spb * CLK_IN_TMR3)//278
#define CNT_per_ONE_BYTE (CNT_per_ONE_BIT*10.0)//5556
#define CNT_per_ONE_TX (CNT_per_ONE_BYTE * 20.0)//111120

uint8_t buffer_Xbee[20] = {};
...
void U1_TX_1Byte(void) {
    TXREG1 = buffer_Xbee[buffer_cnt++];
    setup_TMR3();
}
...
void set_buffer() {
    ...
    buffer_Xbee[0] = 'S';
    buffer_Xbee[1] = 'S';
    buffer_Xbee[2] = (IO_RCrossD_GetValue() << 7) | (IO_RCrossU_GetValue() << 6) | (IO_RCrossR_GetValue() << 5) | (IO_RCrossL_GetValue() << 4)
            | (IO_LCrossD_GetValue() << 3) | (IO_LCrossU_GetValue() << 2) | (IO_LCrossR_GetValue() << 1) | (IO_LCrossL_GetValue() << 0);
    buffer_Xbee[3] = buffer_Xbee[2] ^ 0xFF;

    buffer_Xbee[4] = (IO_RLeverSwR_GetValue() << 7) | (IO_RLeverSwL_GetValue() << 6) | (IO_LLeverSwR_GetValue() << 5) | (IO_LLeverSwL_GetValue() << 4)
            | (IO_RMid_GetValue() << 3) | (IO_RUp_GetValue() << 2) | (IO_LMid_GetValue() << 1) | (IO_LUp_GetValue() << 0);
    buffer_Xbee[5] = buffer_Xbee[4] ^ 0xFF;

    buffer_Xbee[6] = 0xff - (ADC_GetConversion(channel_LHorizon) >> 2);
    buffer_Xbee[7] = buffer_Xbee[6] ^ 0xFF;

    buffer_Xbee[8] = (0xff - (ADC_GetConversion(channel_LVertical) >> 2));
    buffer_Xbee[9] = buffer_Xbee[8] ^ 0xFF;

    buffer_Xbee[10] = 0xff - (ADC_GetConversion(channel_RHorizon) >> 2);
    buffer_Xbee[11] = buffer_Xbee[10] ^ 0xFF;

    buffer_Xbee[12] = (0xff - (ADC_GetConversion(channel_RVertical) >> 2));
    buffer_Xbee[13] = buffer_Xbee[12] ^ 0xFF;

    buffer_Xbee[14] = (IO_RDown_GetValue() << 7) | (IO_LDown_GetValue() << 6) | ((1 - IO_RDipSw_GetValue()) << 1) | (IO_LDipSw_GetValue() << 0);
    buffer_Xbee[15] = buffer_Xbee[14] ^ 0xFF;

    buffer_Xbee[16] = (IO_TactSw12_GetValue() << 5) | (IO_TactSw11_GetValue() << 4) | (IO_TactSw10_GetValue() << 3) | (IO_TactSw02_GetValue() << 2) | (IO_TactSw01_GetValue() << 1) | (IO_TactSw00_GetValue() << 0);
    buffer_Xbee[17] = buffer_Xbee[16] ^ 0xFF;
            ...
            if (!escaped) {
                switch (recieveddata) {
                    ...
                    case '\0':
                        ...
                        //違う行の再表示
                        oled_position(line, str_temp[line].col);
                        for (i = 0; i < 16 - str_temp[line].col; i++) {
                            if (str_temp[line].str[i + str_temp[line].col] != '\0') {
                                oled_data(str_temp[line].str[i + str_temp[line].col]);
                            } else {
                                break;
                            }
                        }
                        ...
                        break;
                    case '\x11':
                        str_temp[line].str[now_pos+1] = '\0';//前の行の表示内容の終了
                        break;
                    case '\x12':
                        break;
                    case '\n':
                    ...
                    default:
                        ...
                        str_temp[line].str[columun] = recieveddata;
                        now_pos = columun++;
                        break;
                }
            }
