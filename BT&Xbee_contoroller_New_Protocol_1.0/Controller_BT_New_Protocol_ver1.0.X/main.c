...
void oled_contrast(uint8_t cont) {
    oled_cmd(0x2A); // RE = 1
    oled_cmd(0x79); // SD = 1
    oled_cmd(0x81); // contrast controljh
    oled_cmd(cont); // value
    oled_cmd(0x78); // SD = 0
    oled_cmd(0x28); // RE = 0
    __delay_ms(2);
...
void EUSART_CompWrite(uint8_t data) {
    EUSART1_Write((data & 0xf0) | (not_of_before_data & 0x0f));
    not_of_before_data = ~data;
    EUSART1_Write((data << 4) | (not_of_before_data >> 4));
}
...
void Bluetooth_Reset(void){
    ...
    __delay_ms(10);
    LATB7=0;
    for(i=0;i<15;i++)__delay_ms(10);
    LATB7=1;

    for(i=0;i<100;i++)__delay_ms(10);//wait 1s
    while(!IO_LAnPush_PORT);
}
...
void main(void) {
    ...
    //Explicit the program is a Bluetooth mode:LED lighting
    LATEbits.LATE2=1;
    ...
    while (1) {
        if (tmrFlag) {
            ...
            EUSART_CompWrite((IO_RLeverSwR_GetValue() << 7) | (IO_RLeverSwL_GetValue() << 6) | (IO_LLeverSwR_GetValue() << 5) | (IO_LLeverSwL_GetValue() << 4)
                    | (IO_RMid_GetValue() << 3) | (IO_RDown_GetValue() << 2) | (IO_LMid_GetValue() << 1) | (IO_LDown_GetValue() << 0));
            ...
            EUSART_CompWrite((IO_RUp_GetValue() << 7) | (IO_LUp_GetValue() << 6) | ((1 - IO_RDipSw_GetValue()) << 1) | (IO_LDipSw_GetValue() << 0));
            ...
            EUSART1_Write(0x30 | (not_of_before_data &0x0f));
            EUSART1_Write('C');
